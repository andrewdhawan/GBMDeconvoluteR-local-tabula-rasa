...
gene_set_list <- readRDS("data/MCPCounter_Immune_Agumented.rds")

# neftel_sigs <- read_rds("data/neftel_sigs.rds")
# 
# foo = data.frame(
#   `HUGO symbols` = unlist(neftel_sigs, use.names = FALSE),
#   `Cell population` = rep(names(neftel_sigs), lengths(neftel_sigs))
#   )
# 
# foo = foo %>% rename(`HUGO symbols` = HUGO.symbols,
#                `Cell population` = Cell.population)
# 
# gene_set_list = rbind(gene_set_list, neftel_sigs)
# 
# 
# saveRDS(gene_set_list, "data/MCPCounter_Immune_Agumented.rds")
